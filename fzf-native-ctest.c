...
static void test_rank_keys_match_fzf_length_and_path_rules(void) {
  ...
  FzfRankKeys default_keys = fzf_rank_keys(
      unicode_space_text, sizeof unicode_space_text - 1, &bounds,
  ...
  FzfRankKeys history_keys = fzf_rank_keys(
      NULL, SIZE_MAX, &bounds, FZF_SCORE_SCHEME_HISTORY);
  ...
  default_keys = fzf_rank_keys(
      long_text, long_length, &bounds, FZF_SCORE_SCHEME_DEFAULT);
  CHECK(default_keys.first == UINT16_MAX);
  free(long_text);

  const char path[] = "src/lib/file.c";
  bounds.min_begin = 8;
  FzfRankKeys path_keys = fzf_rank_keys(
      path, sizeof path - 1, &bounds, FZF_SCORE_SCHEME_PATH);
  CHECK(path_keys.first == 1);
  CHECK(path_keys.second == sizeof path - 1);

  bounds.min_begin = 2;
  path_keys = fzf_rank_keys(
      path, sizeof path - 1, &bounds, FZF_SCORE_SCHEME_PATH);
...
static void test_shared_membership_growth_reuses_immutable_prefix(void) {
  ...
  cache_insert_shared_for_request_abortable(
      &cache, "needle", 24, CaseSmart, true,
