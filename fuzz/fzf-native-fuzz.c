...
static size_t visible_character_count(const char *text) {
  ...
  /* The matcher gives each undecodable byte one surrogate-escape unit.
     Use that same lossy decoder here, so malformed strings get the same
     exact bounds oracle as valid UTF-8 instead of a weaker byte bound. */
  return utf8_strlen(text, bytes);
}
...
static void check_term_positions(const char *candidate, const fzf_term_t *term,
  ...
  /* Valid scalars and undecodable bytes are both stable units under the
     matcher's lossy decoder.  Every matched term must highlight one position
     per decoded pattern unit. */
  if (matched) {
