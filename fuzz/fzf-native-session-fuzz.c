...
static AsyncSession *session_fuzz_create(
    uint8_t options, struct AsyncWorkerPool *shared_pool) {
  ...
  /* Keep whole-result membership independently byte-bounded.  Disabling it
     in half the inputs exposes stable-batch lookup instead of allowing an
     unlimited exact membership entry to mask that path on every round. */
  size_t whole_cache_bytes = (options & 1) ? 0 : 64 * 1024;
  cache_init_limits(
      &s->cache, 1 + ((options >> 1) & 7), whole_cache_bytes);
  batch_cache_init(
      &s->batch_cache,
      batch_cache_sizes[(options >> 4) & 3]);
  s->filter_only_min_pool = filter_only_thresholds[(options >> 6) & 3];
  atomic_init(&s->worker_pool, shared_pool);
  s->worker_pool_owned = false;
  if (!atomic_load_explicit(&s->worker_pool, memory_order_acquire)) {
...
static void session_fuzz_check_pool(FuzzSession *fuzz,
                                    struct AsyncWorkerPool *pool) {
  pthread_mutex_lock(&pool->mu);
  unsigned active = pool->active;
  unsigned count = pool->count;
  bool has_job = pool->job != NULL;
  size_t waiting = pool->waiting;
  bool contended = atomic_load_explicit(
      &pool->contended, memory_order_acquire);
  bool expected_contended = has_job ? waiting > 0 : waiting > 1;
  bool active_job_has_yield_flag =
      !pool->job || pool->job->yield_after_batch == &pool->contended;
  size_t observed_waiting = 0;
  struct AsyncWorkerWaiter *previous = NULL;
  for (struct AsyncWorkerWaiter *waiter = pool->wait_head;
       waiter; waiter = waiter->next) {
    if (waiter->prev != previous || !waiter->job || !waiter->queued ||
        waiter->job->yield_after_batch != &pool->contended) {
      pthread_mutex_unlock(&pool->mu);
      session_fuzz_fail(fuzz, "shared worker wait queue is corrupt");
    }
    previous = waiter;
    observed_waiting++;
  }
  bool tail_matches = previous == pool->wait_tail;
  pthread_mutex_unlock(&pool->mu);

  if (active > count)
    session_fuzz_fail(fuzz, "shared worker count exceeds its capacity");
  if (!has_job && active != 0)
    session_fuzz_fail(fuzz, "idle shared pool retains active job state");
  if (observed_waiting != waiting || !tail_matches)
    session_fuzz_fail(fuzz, "shared worker wait count diverged");
  if (contended != expected_contended)
    session_fuzz_fail(fuzz, "shared worker contention state diverged");
  if (!active_job_has_yield_flag)
    session_fuzz_fail(fuzz, "active shared job lost its yield flag");
}
...
static void session_fuzz_check_invariants(FuzzSession *fuzz) {
  ...
  if (s->cache.count > s->cache.max_entries ||
      s->cache.used_bytes > s->cache.max_bytes)
    session_fuzz_fail(fuzz, "whole-result cache exceeds its budget");
  size_t observed_cache_bytes = 0;
  for (CacheEntry *entry = s->cache.head; entry; entry = entry->next) {
    observed_cache_bytes += entry->bytes;
    if (entry->m_idx) {
      if (entry->m_idx->count != entry->matched_count)
        session_fuzz_fail(fuzz, "cached membership count is incomplete");
      for (size_t i = 0; i < entry->m_idx->count; i++) {
        if (entry->m_idx->idx[i] >= entry->pool_gen ||
            (i > 0 && entry->m_idx->idx[i - 1] >= entry->m_idx->idx[i]))
          session_fuzz_fail(fuzz, "cached membership is not ordered");
      }
    }
  }
  if (observed_cache_bytes != s->cache.used_bytes)
    session_fuzz_fail(fuzz, "whole-result cache byte accounting diverged");
  ...
  bool copy_allocation_failed = false;
  ScoredStr *copied = async_copy_public_result(
      ...
      &copied_source_total, &copy_allocation_failed);
  if (copy_allocation_failed)
    session_fuzz_fail(fuzz, "an owned result copy failed allocation");
...
static void session_fuzz_run(const uint8_t *data, size_t size) {
  ...
  uint8_t options = fuzz_take(&input);
  struct AsyncWorkerPool *pool = async_worker_pool_create(1 + (options & 1));
  if (!pool) return;
  AsyncSession *sessions[2] = {
    session_fuzz_create(options, pool),
    session_fuzz_create((uint8_t)(options ^ 0xA6), pool),
  };
  if (!sessions[0] || !sessions[1]) {
    if (sessions[0]) async_session_destroy(sessions[0]);
    if (sessions[1]) async_session_destroy(sessions[1]);
  ...
  while (input.offset < input.size &&
         operation < SESSION_FUZZ_MAX_OPERATIONS) {
    uint8_t control = fuzz_take(&input);
    FuzzSession *selected = &fuzz[(control >> 3) & 1];
    selected->operation = operation;
    switch (control & 7) {
    case 0:
      session_fuzz_append_literal(selected, &input);
      break;
    case 1:
      session_fuzz_append_run(selected, &input);
      break;
    case 2:
      session_fuzz_submit(selected, &input);
      break;
    case 3:
      session_fuzz_check_invariants(selected);
      break;
    case 4: {
      uint8_t mode = fuzz_take(&input);
      if ((mode & 3) == 0)
        session_fuzz_wait_running(selected);
      else
        for (unsigned i = 0; i < 1 + (mode & 31); i++) sched_yield();
      break;
    }
    case 5:
      session_fuzz_verify(selected);
      break;
    case 6:
      session_fuzz_append_special(selected, &input);
      break;
    case 7:
      ...
      if ((control & 0x80) && selected->submitted) stop_while_active = true;
      else session_fuzz_check_invariants(selected);
      break;
    }
    operation++;
    if (stop_while_active) break;
    for (size_t i = 0; i < 2; i++) {
      fuzz[i].operation = operation;
      session_fuzz_check_invariants(&fuzz[i]);
    }
    session_fuzz_check_pool(selected, pool);
  }

  if (!stop_while_active) {
    for (size_t i = 0; i < 2; i++) {
      atomic_store_explicit(
          &sessions[i]->reader_done, true, memory_order_release);
      atomic_store_explicit(
          &sessions[i]->producer_state, AsyncProducerComplete,
          memory_order_release);
    }
    for (size_t i = 0; i < 2; i++) session_fuzz_verify(&fuzz[i]);
    session_fuzz_check_pool(&fuzz[0], pool);
  }
  async_session_destroy(sessions[0]);
  async_session_destroy(sessions[1]);
  async_worker_pool_destroy(pool);
}
