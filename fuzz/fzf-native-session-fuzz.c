...
static void session_fuzz_check_reference(FuzzSession *fuzz) {
  ...
  for (size_t i = 0; i < pool; i++) {
    ...
    if (score > 0) {
      ...
      if (pattern && !filter_only)
        value.rank = fzf_rank_keys(
            candidate, strlen(candidate), &bounds, score_scheme);
  ...
  if (filter_only && sortable && emit > 1) {
    for (size_t i = 0; i < emit; i++) {
      ...
      reference[i].rank = fzf_rank_keys(
          reference[i].str, strlen(reference[i].str), &bounds, score_scheme);
    }
